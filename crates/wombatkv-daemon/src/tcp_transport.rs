...
//!
//! `[u32 BE length][rkyv envelope]` framing pattern (also used by
//! iggy and similar high-throughput message frames). The default
//! variant is compio + `SO_REUSEPORT` per-shard accept; the module
//! also keeps a `std::net` + `std::thread`-per-connection variant as
//! a lowest-dep simple baseline.
//!
//! ## Server
//!
...
// ONE runtime per transport: `serve_tcp_compio_bridge` (compio +
// SO_REUSEPORT + flume dispatch bridge). Default TPC threads = 2
// so SO_REUSEPORT engages out of the box; set
// `WMBT_KV_TCP_TPC_THREADS=1` for single-thread compio when you
// really want the lowest mem footprint.
...
// ============================================================
// compio io_uring TCP server (per-shard SO_REUSEPORT pattern)
// ============================================================
//
...
//
// Compared to a single-accept-loop / thread-per-connection shape,
// the compio TPC variant gives:
//   - N accept loops (kernel-balanced via SO_REUSEPORT)
//   - one async task per connection (lightweight)
//
...
// ============================================================
// Dispatch bridge: adapts compio acceptors to the worker pool
// ============================================================
//
...
//
// We use `flume::bounded(1)` because flume is the only runtime-
// agnostic primitive in our dep tree and we don't need tokio for
// plain sync workers.
    ...
    F: Fn(u64, WireRequest) -> WireResponse + Send + Sync + 'static,
{
    ...
    // Capacity heuristic: workers × 4 jobs
...
mod tests {
    ...
    /// std::net path was deleted, so all tests use the
    ...
    /// the std::net `boot_tcp_sync_server` was deleted.
