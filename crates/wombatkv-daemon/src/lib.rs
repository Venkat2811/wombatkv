...
///
/// Overridable via `WMBT_KV_DAEMON_SHM_ATTACH_TIMEOUT_SECS` env (alpha.14+)
...
/// the existing `WireRequest.payload` / `WireResponse.payload` `Vec<u8>`
///, no envelope-level change. RFC 0008 §4e completed the migration from
...
/// codec surface (alpha.13+). Named `WireCodecError` to avoid
...
/// Read `WMBT_KV_DAEMON_SHM_WAIT_STRATEGY` env (alpha.12+). Default is `Block`
        ...
        match TypedConsumer::<ShmFrame>::attach_with_consumer_id(
            ...
            Err(error) => {
                if Instant::now() < deadline {
                    // alpha.12+: use myelon's discovery-poll primitive instead
