...
//!
//! Why both? rkyv is compact and zero-copy on decode, which keeps
//! the same wire-level efficiency as the SHM and TCP transports.
//! Wrapping it in HTTP/1.1 lets the daemon sit behind load
//! balancers, reverse proxies, and middleboxes that only speak
//! HTTP, while preserving rkyv's compactness end-to-end.
//!
//! ## Wire format
//!
//! ```text
...
// the std::net `serve_http` + `handle_http_connection`
...
//
// Identical architecture to `tcp_transport::serve_tcp_compio_bridge`, see that module's header for the rationale and bench numbers.
...
mod tests {
    ...
    /// std::net path deleted; tests use compio TPC server.
    ...
    where
        F: Fn(u64, WireRequest) -> WireResponse + Send + Sync + 'static,
    {
        // std::net path deleted; corruption tests run against
