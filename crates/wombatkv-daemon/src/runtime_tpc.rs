...
//!
//! RFC 0007 §10 P4 calls for the daemon to adopt the an earlier internal prototype
...
//! Linux `io_uring` / pinned-shard wins on the table, both of which
//! an earlier internal prototype's RFC 0064 measures as the 2,276 qps cohere HNSW c=64
//! shape.
//!
...
//! another. That is the "puffer-shape" invariant from
//! `the prior playground/ENGINEERING.md` §2.
//!
//! # macOS vs Linux
//!
//! `compio` on macOS uses kqueue, which delivers correctness but NOT
//! the `io_uring` perf wins that an earlier internal prototype's RFC 0064 measures.
//! Production target is Linux + `sched_setaffinity`. Mac dev box
//! sees the SHAPE land cleanly; perf bench must be on Linux to
//! validate the 2,276 qps cohere HNSW c=64 numbers.
//!
...
//!    an earlier internal prototype perf shape.
//!
...
//!  * rkyv on the runtime crossing, see task #154.
