...
pub enum WombatKvFailureClass {
    ...
    TransportSlowWrite,
    /// Daemon ships a corrupted RFC 0018
    ...
    OldBlockV1InBucket,
    /// (RFC 0008 §5 recovery) SlateDB metadata put fails. Daemon
    /// must surface the error or fall back to S3-truth, never silently drop.
    SlateDbWriteFailure,
    /// (RFC 0008 §5 recovery) SlateDB on-disk manifest is corrupt
    ...
    /// Two engines attempt to attach the
...
impl WombatKvFailureClass {
    ...
    pub fn all() -> &'static [Self] {
        ...
        &[
            ...
            TransportPartialReadOnHeader,
            TransportSlowWrite,
            // classes, wire / storage / format / version recovery
            WireEnvelopeCorruption,
            OldSidecarV3InBucket,
            OldBlockV1InBucket,
            SlateDbWriteFailure,
            SlateDbManifestCorruption,
            MultiEnginePrefixConflict,
            // daemon startup configuration recovery
            DaemonTpcOnlyNoShmClient,
...
pub fn schedule_for_class(master_seed: u64, class: WombatKvFailureClass) -> FaultPlan {
    ...
    let events = match class {
        ...
        // wire / storage / format / version recovery classes.
        ...
        WombatKvFailureClass::ResourceDiskFull => vec![ScheduledFault {
            ...
            event: WombatKvFaultEvent::ResourceDiskFull {
                // Fail subsequent disk writes after 1-100 MiB cumulative, spans both small-block and large-block regimes.
