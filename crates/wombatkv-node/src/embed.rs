...
impl<S: ObjectStore> WombatKVKvStore<S> {
    ...
    pub fn put_kv(&self, namespace: &str, key: &str, payload: Bytes) -> Result<(), EmbedError> {
        // DST chaos site (alpha.13-polish):
        ...
        // alpha.13-polish (audit fix #98): emit per-component timing so
    ...
    pub fn put_kv_async_s3(this: Arc<Self>, namespace: &str, key: &str, payload: Bytes) {
        ...
        match std::thread::Builder::new().name(format!("wombatkv-embed-async-s3-{bytes_len}")).spawn(
            move || {
    ...
    /// miss). Critical for diagnosing the "blob too big for RAM" pattern
    ///, e.g. qwen3's pre-allocated 4.7 GiB KV cache against a 2 GiB RAM
...
pub fn emit_timing(func: &str, path: &str, stages: &[(&str, u64)]) {
    // alpha.13: ALWAYS record into the global histogram registry -
