    ...
    fn put_kv_blocks(
        ...
        match self {
            ...
            Self::Remote(client) => {
                ...
                // sequential put_kv calls: the daemon parallel-puts and
                //, critically, updates its in-process metadata index
...
impl Handle {
    ...
    pub fn from_env() -> Result<Self, String> {
        ...
        // alpha.13+: every WMBT_KV_* env var the cabi handle-init
    ...
    /// `wombatkv-daemon` listening on `addr` (e.g. `10.0.0.5:7878`).
    ...
    /// `wombatkv-daemon` listening on `addr` (e.g. `10.0.0.5:7879`).
...
/// `"10.0.0.5:7878"`). The daemon must be running with
...
///
/// `*out_matched_count` is the number of hashes, counted from index 0
///, that are present in the index. Stops at the first miss. The
...
pub extern "C" fn wmbt_kv_lookup_block_prefix(
    ...
    let outcome = catch_unwind(AssertUnwindSafe(|| {
        ...
        // an internal layer), but the metadata index is process-global
        //, we accept any namespace string.
